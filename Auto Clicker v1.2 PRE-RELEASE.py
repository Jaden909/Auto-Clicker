...
# v1.2 (BETA) Added ability to change click times (via UI), not changing all settings and attempting to update values no longer throws a error and works as expected
...
def mainWindow():
        ...
        def updateValue():
            ...
            if 'setClickTimes' in globals():
                ...
                clickTimesValue.set('Current: '+clickAmount)
        ...
        #intial stuff
        title= Label(mainWin, text= 'Auto Clicker v1.2')
        ...
        changelog=Label(mainWin, text= """Patch Notes:
         v1.2 Added ability to change click times, infinite mode, start key, and stop key (via UI), not changing all settings and attempting to update values no longer throws a error and works as expected, increased window size to accomodate new UI elements""", wraplength=520)
        global clickFrequency
